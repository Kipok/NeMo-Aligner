...
def split_qkv_qwen(gathered_mcore_qkv_layer, cfg):
    hidden_size = cfg.hidden_size
    head_num = cfg.num_attention_heads
    num_query_groups = cfg.get("num_query_groups", head_num)
...
def split_qkv_bias_qwen(gathered_mcore_qkv_layer, cfg):
    ...
    qkv_weights = gathered_mcore_qkv_layer
    qkv_weights = qkv_weights.reshape([qkv_total_dim, head_size])
    ...
    q = qkv_weights[q_slice].reshape(-1)
    k = qkv_weights[k_slice].reshape(-1)
    v = qkv_weights[v_slice].reshape(-1)

    return {q_name: q, k_name: k, v_name: v}


def split_fc1_gate_down_qwen(gathered_mcore_fc1, cfg):
...
mcore_te_to_hf = {
    ...
    'model.decoder.layers.{l}.self_attention.linear_qkv.weight': {"tp":0, "hf_func": split_qkv_qwen},
    'model.decoder.layers.{l}.self_attention.linear_qkv.bias': {"tp":0, "hf_func": split_qkv_bias_qwen},
    'model.decoder.layers.{l}.self_attention.linear_qkv.layer_norm_weight': {"hf": "model.layers.{gl}.input_layernorm.weight"},
    'model.decoder.layers.{l}.mlp.linear_fc1.weight': {"tp": 0, "hf_func": split_fc1_gate_down_qwen},
